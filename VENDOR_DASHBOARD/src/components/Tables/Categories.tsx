...
const Categories = () => {
  ...
  const toggleUpdateCategoryModal = async (categoryID) => {
    ...
    if (!isUpdateCategoryModalOpen) {
      const response = await axios.post(
        `http://localhost:5174/ADMIN/api/category/select-category/${categoryID}`,
      );
  ...
  const handleSubmitAddCategory = async (event) => {
    ...
    try {
      const apiUrl = 'http://localhost:5174/ADMIN/api/category/add-category';
  ...
  const handleSubmitUpdateCategory = async (event) => {
    ...
    if (sUpdateCategoryName == '') {
      ...
      const response = await axios.patch(
        `http://localhost:5174/ADMIN/api/category/update-category/${iUpdateCategoryID}`,
        formUpdateCategory,
      );
      if (response.statusText == 'OK') {
    ...
    if (connfirm) {
      try {
        const response = await fetch(
          `http://localhost:5174/ADMIN/api/category/delete-category/${categoryID}`,
          {
            method: 'DELETE',
          },
        );
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const response = await axios.post(
          `http://localhost:5174/ADMIN/api/category/select-category?page=${currentCategoryPage}&limit=${categoriesPerPage}&search=${searchCategory}`,
        );
        if (response.statusText == 'OK') {
