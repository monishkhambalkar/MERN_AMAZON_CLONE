...
const SubCategories = () => {
  ...
  const handleSubmitUpdateSubCategory = async (event) => {
    ...
    if (sUpdateSubCategoryName == '') {
      ...
      const response = await axios.patch(
        `http://localhost:5174/ADMIN/api/subCategory/update-sub-category/${iUpdateSubCategoryID}`,
    ...
    if (connfirm) {
      try {
        const response = await axios.delete(
          `http://localhost:5174/ADMIN/api/subCategory/delete-sub-category/${subCategoryID}`,
        );
        if (response.statusText == 'OK') {
          alertNotificationFunction(
            'success',
    ...
    const fetchSubCategoryData = async () => {
      try {
        const response = await axios.post(
          `http://localhost:5174/ADMIN/api/subCategory/select-sub-category?page=${currentSubCategoryPage}&limit=${subCategoryPerPage}&search=${searchSubCategory}`,
        );
        if (response.statusText == 'OK') {
  ...
  const toggleUpdateSubCategoryModal = async (subCategoryID) => {
    ...
    if (!isUpdateSubCategoryModalOpen) {
      const response = await axios.post(
        `http://localhost:5174/ADMIN/api/subCategory/select-sub-category/${subCategoryID}`,
      );
      if (response.statusText == 'OK') {
  ...
  const handleSubmitAddSubCategory = async (event) => {
    ...
    const response = await axios.post(
      `http://localhost:5174/ADMIN/api/subCategory/add-sub-category`,
      formAddSubCategory,
    );
    console.log(response);
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      try {
        const response = await axios.post(
          `http://localhost:5174/ADMIN/api/category/select-category?page=0&limit=100`,
        );
        if (response.statusText == 'OK') {
