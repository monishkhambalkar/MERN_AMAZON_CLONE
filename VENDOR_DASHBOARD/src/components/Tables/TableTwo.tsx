    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.post(
          `http://localhost:5174/ADMIN/api/product/select-product?page=${currentProductPage}&limit=${productsPerPage}&search=${productSearch}`,
        );
  ...
  const handleDeleteProduct = async (productID) => {
    ...
    try {
      const deleteProduct = await axios.delete(
        `http://localhost:5174/ADMIN/api/product/delete-product/${productID}`,
      );
