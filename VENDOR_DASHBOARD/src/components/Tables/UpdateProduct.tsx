...
const UpdateProduct = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.patch(
        `http://localhost:5174/ADMIN/api/product/update-product/${id}`,
        formDataToSend,
        {
          headers: {
    ...
    const fetchProductByID = async () => {
      try {
        const response = await axios.post(
          `http://localhost:5174/ADMIN/api/product/select-product/${id}`,
        );
  ...
  useEffect(() => {
    ...
    const fetchSubCategoryData = async () => {
      try {
        const response = await axios.post(
          `http://localhost:5174/ADMIN/api/subCategory/select-sub-category?page=1&limit=100`,
        );
        if (response.statusText == 'OK') {
  ...
  const handleSelectCategory = async (e) => {
    ...
    if (selectedCategoryId !== '') {
      const response = await axios.post(
        `http://localhost:5174/ADMIN/api/subCategory/select-sub-category-by-category/${selectedCategoryId}`,
      );
      if (response.status === 200) {
